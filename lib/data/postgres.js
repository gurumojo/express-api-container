...
function bootstrap(pgOpts, dbOpts) {
	...
	.then(pg => {
		release(pg);
		logger.debug(`${namespace}.ready`, dbOptions);
	})
	.catch(e => {
...
function applyPatches(pg) {
	return Promise.mapSeries(sql.patch, query => {
		return pg.none(query).then(() => {
			logger.debug(`${namespace}.patch`, {applied: query.file.split('/').pop()});
		});
	})
	.then(() => pg);
}
...
function revertPatches(pg) {
	return Promise.mapSeries(sql.purge, query => {
		return pg.none(query).then(() => {
			logger.debug(`${namespace}.purge`, {reverted: query.file.split('/').pop()});
		});
	})
	.then(() => pg);
}
